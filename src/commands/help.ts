...
export function printRenderHelp(): void {
  const lines = [
    ...
    "      --poster <time|auto>  Write one PNG still at that time (seconds) instead of a video;",
    "                            `auto` is mid-duration. Captures a handful of frames, not all of them",
...
export function printLintHelp(): void {
  const lines = [
    ...
    "  hfmpeg lint ./my-video --hermetic",
    "  hfmpeg lint ./my-video --json",
    "",
    "Flags:",
    "  -c, --composition <file>  Lint a specific entry file (default: index.html)",
    "      --strict              Exit non-zero on warnings as well as errors",
    "      --hermetic            Remote script/font/media references become errors (offline-safe check)",
